...
def main():
    ...
    if "db_conn" not in st.session_state:
        ...
        st.session_state.docx_uploader_key = 1
        st.session_state.pptx_uploader_key = 2
        st.session_state.xlsx_uploader_key = 3
    ...
    # File uploaders for different file types
    ...
    uploaded_docx = st.sidebar.file_uploader("Upload a DOCX file", accept_multiple_files=True, key=st.session_state.docx_uploader_key, type=["docx"], on_change=toggle_docx_chat)
    uploaded_xlsx = st.sidebar.file_uploader("Upload an XLSX file", accept_multiple_files=True, key=st.session_state.xlsx_uploader_key, type=["xlsx"], on_change=toggle_xlsx_chat)
    uploaded_pptx = st.sidebar.file_uploader("Upload a PPTX file", accept_multiple_files=True, key=st.session_state.pptx_uploader_key, type=["pptx"], on_change=toggle_pptx_chat)
    ...
    if uploaded_docx:
        with st.spinner("Processing DOCX..."):
            ...
            st.session_state.docx_uploader_key += 2

    if uploaded_xlsx:
        with st.spinner("Processing XLSX..."):
            add_documents_to_db(uploaded_xlsx, "xlsx")
            st.session_state.xlsx_uploader_key += 2
